...
class Args:
    ...
    sparsity: float = 0.5
    """fraction of weights to prune (set to 0 at initialization)"""
...
if __name__ == "__main__":
    ...
    # Network sparsity: create random binary masks and apply them at initialization.
    # Each mask entry is 1 (keep) with probability (1 - sparsity) and 0 (prune) otherwise.
    sparse_masks = {}
    if args.sparsity > 0.0:
        with torch.no_grad():
            for name, param in agent.named_parameters():
                mask = (torch.rand_like(param) < (1.0 - args.sparsity)).to(device)
                sparse_masks[name] = mask
                param.mul_(mask)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Re-apply sparsity masks after each optimizer step to keep pruned weights at zero.
                if sparse_masks:
                    with torch.no_grad():
                        for name, param in agent.named_parameters():
                            if name in sparse_masks:
                                param.mul_(sparse_masks[name])
    ...
    if all_episode_returns:
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            w.writerow([args.env_id, args.seed, args.hypothesis_id, args.experiment_id, "ppo_sparse",
