# PPO + Muon Optimizer (h019)
# Based on ppo_atari_envpool.py with the Muon optimizer replacing Adam.
# Muon applies Newton-Schulz orthogonalization to gradients of matrix-shaped
# parameters (Conv2d, Linear weights) and falls back to AdamW for 1D params.
import csv
import os
import random
import time
...
# ---------------------------------------------------------------------------
# Muon optimizer — inline implementation
# ---------------------------------------------------------------------------

def zeropower_via_newtonschulz5(G, steps=5, eps=1e-7):
    """Compute G @ (G^T @ G)^{-1/2} via Newton-Schulz iteration."""
...
class Muon(Optimizer):
    """Muon optimizer: Newton-Schulz orthogonalised updates for matrix params,
    AdamW for scalar/1-D params (biases, layer-norm, etc.)."""

    def __init__(self, muon_params, lr=0.02, momentum=0.95, nesterov=True,
                 adam_params=None, adam_lr=3e-4, adam_betas=(0.9, 0.999), adam_eps=1e-8):
        defaults = dict(lr=lr, momentum=momentum, nesterov=nesterov)
        params = list(muon_params)
        super().__init__(params, defaults)
        self.adam = (
            torch.optim.AdamW(adam_params, lr=adam_lr, betas=adam_betas, eps=adam_eps)
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if g.ndim >= 2:
                    ...
                    g_2d = zeropower_via_newtonschulz5(g_2d)
                    g = g_2d.view_as(g)
                    p.add_(g, alpha=-lr)
                else:
                    # For 1D params, use simple SGD with momentum
                    state = self.state[p]
                    if len(state) == 0:
                        state['momentum_buffer'] = torch.zeros_like(g)
                    buf = state['momentum_buffer']
...
class Args:
    ...
    muon_lr: float = 0.02
    """learning rate for Muon (matrix-shaped params)"""
    adam_lr: float = 3e-4
    """learning rate for AdamW (1-D params like biases)"""
    num_envs: int = 128
...
if __name__ == "__main__":
    ...
    # --- Split parameters: matrix params -> Muon, 1-D params -> AdamW ---
    matrix_params = []
    adam_params = []
    ...
    # Collect all parameters for grad clipping
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.anneal_lr:
            ...
            # Scale both Muon and Adam LRs proportionally
            optimizer.param_groups[0]["lr"] = frac * args.muon_lr
            if optimizer.adam:
                optimizer.adam.param_groups[0]["lr"] = frac * args.adam_lr
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Zero gradients for both Muon params and Adam params
                optimizer.zero_grad()
                if optimizer.adam:
                ...
                # Clip gradients across ALL parameters together
                nn.utils.clip_grad_norm_(all_params, args.max_grad_norm)

                # Step both Muon (matrix params) and internal Adam (1-D params)
                optimizer.step()
        ...
        writer.add_scalar("charts/muon_lr", optimizer.param_groups[0]["lr"], global_step)
        if optimizer.adam:
            writer.add_scalar("charts/adam_lr", optimizer.adam.param_groups[0]["lr"], global_step)
    ...
    if all_episode_returns:
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            w.writerow([args.env_id, args.seed, args.hypothesis_id, args.experiment_id, "ppo_muon",
