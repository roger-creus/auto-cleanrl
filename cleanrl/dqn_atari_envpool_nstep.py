# DQN with N-step returns for Atari using envpool
# Based on dqn_atari_envpool.py — uses multi-step bootstrapped returns (Rainbow paper: n=3 optimal)
import csv
import os
import random
import time
...
class NStepBuffer:
    ...
    def add(self, env_idx, obs, action, reward, done, next_obs, replay_buffer):
        ...
        Args:
            ...
            obs: observation at current step
            action: action taken
            reward: reward received
            done: whether episode ended
            next_obs: observation after action (used as bootstrap state for full deque)
            replay_buffer: the SimpleReplayBuffer to emit transitions into
        """
        self.buffers[env_idx].append((obs, action, reward, done))

        if done:
            # Episode ended: flush all partial n-step transitions from the deque
            self._flush(env_idx, next_obs, replay_buffer)
        elif len(self.buffers[env_idx]) == self.n_step:
            # Deque is full: emit one n-step transition
            ...
            # next_obs is the state n steps after obs_0 (the observation we just received)
    ...
    def _flush(self, env_idx, final_next_obs, replay_buffer):
        ...
        Called when an episode ends. Computes partial n-step returns for all
        remaining transitions and adds them to the replay buffer.
        """
        ...
        while len(buf) > 0:
            ...
            obs_0 = transitions[0][0]
            action_0 = transitions[0][1]
            # For a terminal flush, the "next_obs" is the final next_obs
            # (the auto-reset observation), and done_any should be True
...
if __name__ == "__main__":
    ...
    # N-step transition buffer (one deque per environment)
    nstep_buffer = NStepBuffer(args.num_envs, args.n_step, args.gamma)

    # Precompute gamma^n for the bootstrap target
    ...
    while global_step < args.total_timesteps:
        ...
        # Add transitions through n-step buffer (which emits to replay buffer when ready)
    ...
    if all_episode_returns:
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            w.writerow([args.env_id, args.seed, args.hypothesis_id, args.experiment_id, "dqn_nstep",
