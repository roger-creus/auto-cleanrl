# IQN + OQE + N-step returns for Atari using envpool (NO NoisyNet)
# Tests whether novel OQE exploration can replace NoisyNet in Rainbow-lite.
# OQE provides directed optimistic exploration via upper-quantile action selection.
# N-step returns improve credit assignment. Epsilon-greedy for undirected exploration.
import csv
import os
import random
import time
...
class Args:
    ...
    oqe_start: float = 0.5
    """initial lower bound for tau sampling during action selection (0.5 = upper-half only)"""
    oqe_end: float = 0.0
    """final lower bound for tau sampling (0.0 = standard full-distribution mean)"""
    oqe_fraction: float = 0.5
    """fraction of training over which to anneal oqe_start -> oqe_end"""

    # results tracking
    hypothesis_id: str = "h070"
...
if __name__ == "__main__":
    ...
    while global_step < args.total_timesteps:
        ...
        with torch.no_grad():
            ...
            # OQE: sample tau from U(oqe_beta, 1.0) instead of U(0, 1)
    ...
    if all_episode_returns:
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            w.writerow([args.env_id, args.seed, args.hypothesis_id, args.experiment_id, "iqn_oqe_nstep",
