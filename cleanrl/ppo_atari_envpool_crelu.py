...
class CReLU(nn.Module):
    """Concatenated ReLU: CReLU(x) = [ReLU(x), ReLU(-x)], doubling channels."""
...
class Agent(nn.Module):
    def __init__(self, envs):
        super().__init__()
        # CReLU doubles channels, so we halve conv output channels to match
        # original parameter count:
        # Conv1: 4→16, CReLU → 32 (was 4→32, ReLU → 32)
        # Conv2: 32→32, CReLU → 64 (was 32→64, ReLU → 64)
        # Conv3: 64→32, CReLU → 64 (was 64→64, ReLU → 64)
        # Linear: 3136→256, CReLU → 512 (was 3136→512, ReLU → 512)
        self.network = nn.Sequential(
            layer_init(nn.Conv2d(4, 16, 8, stride=4)),
            CReLU(dim=1),
            layer_init(nn.Conv2d(32, 32, 4, stride=2)),
            CReLU(dim=1),
...
if __name__ == "__main__":
    ...
    if all_episode_returns:
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            w.writerow([args.env_id, args.seed, args.hypothesis_id, args.experiment_id, "ppo_crelu",
