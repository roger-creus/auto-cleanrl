...
# PPO + DrQ-style random shift augmentation + augmentation consistency auxiliary loss
import csv
import os
import random
import time
...
class Args:
    ...
    consist_coef: float = 0.5
    """coefficient of the augmentation consistency auxiliary loss"""
...
class Agent(nn.Module):
    ...
    def get_features(self, x):
        """Return CNN encoder features (512-dim) before actor/critic heads."""
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                mb_obs_orig = b_obs[mb_inds]
                mb_obs_aug = random_shift(mb_obs_orig, pad=args.aug_pad)
                ...
                # Augmentation consistency loss: pull augmented features toward clean features.
                # Asymmetric update (BYOL-style): only the augmented branch receives gradients,
                # the original branch is treated as the target via .detach().
                features_orig = agent.get_features(mb_obs_orig).detach()
                features_aug = agent.get_features(mb_obs_aug)
                consist_loss = F.mse_loss(features_aug, features_orig)

                loss = (pg_loss
                        - args.ent_coef * entropy_loss
                        + v_loss * args.vf_coef
                        + args.consist_coef * consist_loss)
        ...
        writer.add_scalar("losses/consistency_loss", consist_loss.item(), global_step)
    ...
    if all_episode_returns:
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            w.writerow([args.env_id, args.seed, args.hypothesis_id, args.experiment_id, "ppo_aug_consist",
