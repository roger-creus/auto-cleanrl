...
class Args:
    ...
    chain_coef: float = 0.1
    """coefficient for CHAIN churn reduction loss"""
    chain_ref_ratio: float = 0.5
    """fraction of batch to use as reference states for CHAIN"""
...
class Agent(nn.Module):
    def __init__(self, envs):
        super().__init__()
        # NatureCNN with LayerNorm after each conv and linear layer
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # CHAIN: Pre-compute reference outputs before any updates this iteration
        n_ref = int(args.batch_size * args.chain_ref_ratio)
        ref_inds = np.random.choice(args.batch_size, size=n_ref, replace=False)
        with torch.no_grad():
            ref_hidden = agent.network(b_obs[ref_inds] / 255.0)
            ref_values = agent.critic(ref_hidden).squeeze(-1)
            ref_logits = agent.actor(ref_hidden)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # CHAIN loss: penalize churn on reference states
                cur_ref_hidden = agent.network(b_obs[ref_inds] / 255.0)
                cur_ref_values = agent.critic(cur_ref_hidden).squeeze(-1)
                cur_ref_logits = agent.actor(cur_ref_hidden)
                chain_value_loss = F.mse_loss(cur_ref_values, ref_values)
                chain_policy_loss = F.mse_loss(cur_ref_logits, ref_logits)
                chain_loss = chain_value_loss + chain_policy_loss
                chain_losses.append(chain_loss.item())

                entropy_loss = entropy.mean()
                loss = pg_loss - args.ent_coef * entropy_loss + v_loss * args.vf_coef + args.chain_coef * chain_loss
        ...
        if chain_losses:
            writer.add_scalar("losses/chain_loss", np.mean(chain_losses), global_step)
    ...
    if all_episode_returns:
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            w.writerow([args.env_id, args.seed, args.hypothesis_id, args.experiment_id, "ppo_chainsp",
