...
# NaP+LSTM variant: combines Normalize-and-Project (h004) with LSTM (h008)
import csv
import os
import random
import time
...
class Args:
    ...
    """the maximum norm for the gradient clipping (NaP controls norm growth so higher value is appropriate)"""
    start_e: float = 1
...
class QNetwork(nn.Module):
    def __init__(self, env):
        super().__init__()
        # CNN feature extractor: 4 stacked frames input (same as base PQN and NaP variant)
        ...
        # LSTM: bridges CNN output and Q-head, carries temporal context across steps
        ...
        # Q-head: maps LSTM hidden state to action values
        self.q_func = layer_init(nn.Linear(128, env.single_action_space.n))

    def get_states(self, x, lstm_state, done):
        hidden = self.network(x / 255.0)

        # LSTM logic: reset hidden state for envs that are done
...
class NaP:
    """Normalize-and-Project: maintain constant effective learning rate.

    After each optimizer step, project each weight matrix back to its initial
    Frobenius norm. This prevents parameter norm growth from decaying the
    effective learning rate when using LayerNorm.

    Reference: "Normalize-and-Project" (NeurIPS 2024)

    NOTE: NaP is intentionally NOT applied to LSTM weights. LSTM gates rely on
    carefully scaled weights for proper gating behavior; projecting them back to
    initial norms could disrupt learned gate dynamics. NaP is applied only to
    the CNN linear layer (network) and the Q-head linear layer (q_func).
    """
    def __init__(self, model):
        self.initial_norms = {}
        for name, param in model.named_parameters():
            # Only project weight matrices (dim >= 2), skip biases and LSTM weights
            if param.dim() >= 2 and not name.startswith("lstm."):
                self.initial_norms[name] = param.data.norm().item()

    @torch.no_grad()
    def project(self, model):
        for name, param in model.named_parameters():
            if name in self.initial_norms:
                current_norm = param.data.norm().item()
                if current_norm > 0:
                    param.data.mul_(self.initial_norms[name] / current_norm)
...
if __name__ == "__main__":
    ...
    # Initialize NaP after model is on device — records initial Frobenius norms
    # of CNN and Q-head weight matrices only (LSTM weights excluded)
    nap = NaP(q_network)
    ...
    # LSTM hidden and cell states: (num_layers, num_envs, hidden_size)
    next_lstm_state = (
    ...
    for iteration in range(1, args.num_iterations + 1):
        # Save LSTM state at the start of each rollout for use during training
        ...
        # LSTM-aware minibatch indexing: split along env dimension so each
        # minibatch gets a contiguous sequence of steps for its envs, allowing
        # the initial LSTM state to be fed correctly into the forward pass.
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.num_envs, envsperbatch):
                ...
                optimizer.step()
                # NaP: project CNN and Q-head weights back to initial Frobenius norms
                # This prevents norm growth from shrinking the effective learning rate.
                # LSTM weights are deliberately excluded (see NaP class docstring).
                nap.project(q_network)
    ...
    if all_episode_returns:
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            w.writerow([args.env_id, args.seed, args.hypothesis_id, args.experiment_id, "pqn_nap_lstm",
