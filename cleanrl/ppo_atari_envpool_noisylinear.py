...
# NoisyLinear variant: replaces actor/critic heads with NoisyLinear layers (Fortunato et al. 2018)
import csv
import os
import random
import time
...
class Args:
    ...
    """coefficient of the entropy (reduced; noise replaces entropy-based exploration)"""
    vf_coef: float = 0.5
...
class Agent(nn.Module):
    def __init__(self, envs):
        super().__init__()
        # NatureCNN encoder — unchanged from baseline
        ...
        # Actor head: NoisyLinear(512, 512) -> ReLU -> NoisyLinear(512, n_actions)
        self.actor_head = nn.Sequential(
            NoisyLinear(512, 512),
            nn.ReLU(),
            NoisyLinear(512, envs.single_action_space.n),
        )
        # Critic head: NoisyLinear(512, 512) -> ReLU -> NoisyLinear(512, 1)
    ...
    def get_value(self, x):
        return self.critic_head(self.network(x / 255.0))
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            # Resample noise at the start of each update epoch
            agent.reset_noise()
    ...
    if all_episode_returns:
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            w.writerow([args.env_id, args.seed, args.hypothesis_id, args.experiment_id, "ppo_noisylinear",
