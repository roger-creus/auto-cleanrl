# PPO + LSTM for Atari using envpool
# Combines LSTM temporal memory from ppo_atari_lstm.py with envpool from ppo_atari_envpool.py
import csv
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, lstm_hidden_size=128):
        super().__init__()
    ...
    def get_states(self, x, lstm_state, done):
        ...
        # LSTM logic: process sequentially, resetting state on done
...
if __name__ == "__main__":
    ...
    agent = Agent(envs, lstm_hidden_size=args.lstm_hidden_size).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # LSTM requires per-env sequence sampling (not random flat indices)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.num_envs, envsperbatch):
                ...
                mb_inds = flatinds[:, mbenvinds].ravel()  # maintain temporal order per env
    ...
    if all_episode_returns:
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            w.writerow([args.env_id, args.seed, args.hypothesis_id, args.experiment_id, "ppo_lstm",
