# PPO + CVaR + Simplicial Embeddings (SEM) + DrQ Augmentation
# Combines h029 (CVaR advantage estimation) with h030 (simplicial embeddings)
# SEM replaces ReLU penultimate layer with product-of-softmax groups → structured sparse representations
# CVaR provides exploration-exploitation scheduling via pessimistic value baselines
import csv
import os
import random
import time
...
class Args:
    ...
    # SEM parameters
    sem_groups: int = 32
    """number of softmax groups for simplicial embeddings"""
    sem_vertices: int = 16
    """number of vertices per group (total dim = groups * vertices)"""
...
class Agent(nn.Module):
    def __init__(self, envs, num_quantiles=32, sem_groups=32, sem_vertices=16, sem_temperature=0.1):
        ...
        # Quantile value head: outputs num_quantiles values
        self.critic = layer_init(nn.Linear(sem_dim, num_quantiles), std=1)
    ...
    def get_value(self, x, cvar_alpha=1.0):
        """Return CVaR value estimate from SEM features."""
        quantiles = self.critic(self._sem_embed(x))
...
if __name__ == "__main__":
    ...
    if all_episode_returns:
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            w.writerow([args.env_id, args.seed, args.hypothesis_id, args.experiment_id, "ppo_cvar_sem",
