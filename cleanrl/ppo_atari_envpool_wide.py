# PPO with Wider NatureCNN for Atari using envpool
# Doubles conv channel widths: 32→64, 64→128, 64→128 and hidden: 512→1024
# More parameters = more representational capacity for complex games
import csv
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs):
        super().__init__()
        # WIDER NatureCNN: doubled channel widths (32→64, 64→128, 64→128) and hidden (512→1024)
        self.network = nn.Sequential(
            layer_init(nn.Conv2d(4, 64, 8, stride=4)),
            nn.ReLU(),
            ...
            nn.ReLU(),
            nn.Flatten(),
            layer_init(nn.Linear(128 * 7 * 7, 1024)),
            nn.ReLU(),
        )
...
if __name__ == "__main__":
    ...
    if all_episode_returns:
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            w.writerow([args.env_id, args.seed, args.hypothesis_id, args.experiment_id, "ppo_wide",
