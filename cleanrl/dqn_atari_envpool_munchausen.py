# Munchausen DQN for Atari using envpool — adds Munchausen reward bonus to the TD target
# Based on dqn_atari_envpool.py; see Vieillard et al. NeurIPS 2020 "Munchausen Reinforcement Learning"
import csv
import os
import random
import time
...
class Args:
    ...
    """scaling coefficient for the Munchausen log-policy bonus"""
    munchausen_tau: float = 0.03
    """temperature for the soft policy / log-policy bonus"""
    munchausen_clip: float = -1.0
    """lower clip value for the log-policy term (prevents large negative bonuses)"""
    double_dqn: bool = True
    """use Double DQN (online network selects action, target network evaluates)"""
...
if __name__ == "__main__":
    ...
    while global_step < args.total_timesteps:
        ...
        if global_step > args.learning_starts:
            if (global_step // args.num_envs) % args.train_frequency == 0:
                ...
                with torch.no_grad():
                    # --- Munchausen DQN target ---
                    # Q-values from target network at next states
                    target_q_next = target_network(s_next_obs)

                    if args.double_dqn:
                        # Double DQN with soft value: online network selects the action,
                        # but we use the Munchausen soft value (logsumexp) for the bootstrap.
                        # The soft value already marginalises over all actions so Double DQN
                        # does not add extra benefit here; we keep the flag for completeness
                        # and fall through to the logsumexp target below.
                        pass

                    # Soft value V(s') = tau * log(sum_a exp(Q(s',a) / tau))
                    target_v_next = args.munchausen_tau * torch.logsumexp(
                        target_q_next / args.munchausen_tau, dim=1
                    )

                    # Munchausen bonus: alpha * clip(tau * log pi(a | s), clip, 0)
                    # where log pi(a|s) = Q(s,a)/tau - logsumexp(Q(s,·)/tau)  (softmax log-policy)
                    target_q_curr = target_network(s_obs)
                    log_pi = (
                        target_q_curr / args.munchausen_tau
                        - torch.logsumexp(target_q_curr / args.munchausen_tau, dim=1, keepdim=True)
                    )
                    log_pi_a = log_pi.gather(1, s_actions.unsqueeze(1)).squeeze(1)
                    munch_bonus = args.munchausen_alpha * torch.clamp(
                        args.munchausen_tau * log_pi_a,
                        min=args.munchausen_clip,
                        max=0.0,
                    )

                    td_target = s_rewards + munch_bonus + args.gamma * target_v_next * (1 - s_dones)
                ...
                if (global_step // args.num_envs) % 100 == 0:
                    ...
                    writer.add_scalar("losses/munchausen_bonus", munch_bonus.mean().item(), global_step)
    ...
    if all_episode_returns:
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            w.writerow([args.env_id, args.seed, args.hypothesis_id, args.experiment_id, "dqn_munchausen",
