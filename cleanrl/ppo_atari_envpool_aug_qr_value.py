# PPO with DrQ-style Data Augmentation + Quantile Regression Value Function
# Combines two complementary techniques:
# 1. DrQ random shift augmentation during training for improved representation learning
# 2. Quantile regression value head for distributional value estimation
# The mean of quantile predictions serves as V(s) for advantage estimation
# Training uses quantile Huber loss on augmented observations
import csv
import os
import random
import time
...
class Agent(nn.Module):
    ...
    def get_value(self, x):
        """Return mean of quantile predictions as scalar value estimate."""
        quantiles = self.critic(self.network(x / 255.0))  # (batch, num_quantiles)
        return quantiles.mean(dim=-1, keepdim=True)  # (batch, 1)
    ...
    def get_action_and_value(self, x, action=None):
        ...
        value = quantiles.mean(dim=-1, keepdim=True)  # (batch, 1)
        return action, probs.log_prob(action), probs.entropy(), value

    def get_action_value_and_quantiles(self, x, action=None):
        """Return action, log_prob, entropy, scalar value, and full quantile predictions."""
        hidden = self.network(x / 255.0)
        logits = self.actor(hidden)
        probs = Categorical(logits=logits)
        if action is None:
...
if __name__ == "__main__":
    ...
    if all_episode_returns:
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            w.writerow([args.env_id, args.seed, args.hypothesis_id, args.experiment_id, "ppo_aug_qr_value",
