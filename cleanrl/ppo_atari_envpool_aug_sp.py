...
# PPO + DrQ-style random shift data augmentation + Soft Shrink-and-Perturb (SP) regularization.
# Augmentation: random shift applied to observations during training only (not during rollout).
# SP: after each optimizer.step(), weight matrices are shrunk toward zero and perturbed
# with small Gaussian noise. Biases are not affected.
import csv
import os
import random
import time
...
class Args:
    ...
    # Soft Shrink-and-Perturb hyperparameters
    sp_shrink: float = 1e-4
    """shrink coefficient — soft L2 toward origin applied post-step (weight matrices only)"""
    sp_perturb: float = 1e-5
    """perturbation noise std — Gaussian noise added post-step (weight matrices only)"""
...
def random_shift(x, pad=4):
    ...
    """
    n, c, h, w = x.shape
    # Random shifts in [-pad, pad] normalized to [-1, 1] grid coordinates
    ...
    # Create identity grid and apply shifts
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            # ALGO LOGIC: action logic (NO augmentation during rollout)
            with torch.no_grad():
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue = agent.get_action_and_value(mb_obs_aug, b_actions.long()[mb_inds])
                ...
                # Soft Shrink-and-Perturb: maintain plasticity by preventing weight
                # magnitude collapse and injecting small noise. Applied only to weight
                # matrices (dim >= 2); biases are left unchanged.
                with torch.no_grad():
                    for name, param in agent.named_parameters():
                        if param.requires_grad and param.dim() >= 2:
                            # Shrink: soft L2 toward origin (post-step weight decay)
                            param.mul_(1.0 - args.sp_shrink)
                            # Perturb: add small Gaussian noise
                            param.add_(torch.randn_like(param) * args.sp_perturb)
    ...
    if all_episode_returns:
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            w.writerow([args.env_id, args.seed, args.hypothesis_id, args.experiment_id, "ppo_aug_sp",
