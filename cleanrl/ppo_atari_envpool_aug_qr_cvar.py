# PPO with DrQ Augmentation + Quantile Regression + CVaR Advantage (NOVEL)
# Novel contribution: Risk-sensitive advantage estimation using distributional value function
# Instead of A = R - mean(quantiles), uses A = R - CVaR_alpha(quantiles)
# CVaR (Conditional Value at Risk) emphasizes lower quantiles → pessimistic value baseline
# This creates OPTIMISTIC advantages (since baseline is conservative), encouraging exploration
# The alpha parameter anneals from 0.25 (aggressive exploration) to 1.0 (standard mean)
# over training, naturally transitioning from exploration to exploitation
import csv
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, num_quantiles=32):
        super().__init__()
        ...
        # Quantile value head: outputs num_quantiles values instead of 1
        self.critic = layer_init(nn.Linear(512, num_quantiles), std=1)

    def get_value(self, x, cvar_alpha=1.0):
        """Return CVaR value estimate: mean of lowest alpha fraction of quantiles.
        When alpha=1.0, this is the standard mean (all quantiles)."""
        quantiles = self.critic(self.network(x / 255.0))  # (batch, num_quantiles)
        sorted_q, _ = torch.sort(quantiles, dim=-1)  # sort ascending
        k = max(1, int(cvar_alpha * self.num_quantiles))
        return sorted_q[:, :k].mean(dim=-1, keepdim=True)  # (batch, 1)

    def get_quantile_values(self, x):
        """Return all quantile predictions."""
        return self.critic(self.network(x / 255.0))  # (batch, num_quantiles)
    ...
    def get_action_and_value(self, x, action=None, cvar_alpha=1.0):
        ...
        if action is None:
            action = probs.sample()
        quantiles = self.critic(hidden)  # (batch, num_quantiles)
        sorted_q, _ = torch.sort(quantiles, dim=-1)
        k = max(1, int(cvar_alpha * self.num_quantiles))
        value = sorted_q[:, :k].mean(dim=-1, keepdim=True)  # (batch, 1)
    ...
    def get_action_value_and_quantiles(self, x, action=None):
        ...
        if action is None:
            action = probs.sample()
        quantiles = self.critic(hidden)  # (batch, num_quantiles)
        value = quantiles.mean(dim=-1, keepdim=True)  # (batch, 1) — always mean for training loss
...
if __name__ == "__main__":
    ...
    agent = Agent(envs, num_quantiles=args.num_quantiles).to(device)
    ...
    if all_episode_returns:
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            w.writerow([args.env_id, args.seed, args.hypothesis_id, args.experiment_id, "ppo_aug_qr_cvar",
