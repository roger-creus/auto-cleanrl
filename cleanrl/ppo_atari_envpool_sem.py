# PPO + Simplicial Embeddings (SEM)
# Based on "Simplicial Embeddings Improve Sample Efficiency in Actor-Critic Agents" (arXiv:2510.13704)
# Replaces penultimate layer with product-of-softmax groups for structured sparse representations
import csv
import os
import random
import time
...
class Args:
    ...
    # SEM hyperparameters
    sem_groups: int = 32
    """number of simplicial groups L"""
    sem_vertices: int = 16
    """number of vertices per group V (output dim = L*V = 512)"""
    sem_temperature: float = 0.1
    """softmax temperature for simplicial embedding"""
...
class Agent(nn.Module):
    def __init__(self, envs, sem_groups=32, sem_vertices=16, sem_temperature=0.1):
        super().__init__()
        ...
        self.actor = layer_init(nn.Linear(sem_dim, envs.single_action_space.n), std=0.01)
        self.critic = layer_init(nn.Linear(sem_dim, 1), std=1)

    def _sem_embed(self, x):
        """Apply simplicial embedding: reshape to groups and per-group softmax."""
        h = self.cnn(x / 255.0)
        h = self.fc(h)  # (batch, L*V)
        h = h.view(-1, self.sem_groups, self.sem_vertices)  # (batch, L, V)
        h = torch.softmax(h / self.sem_temperature, dim=-1)  # per-group softmax
        h = h.view(-1, self.sem_groups * self.sem_vertices)  # (batch, L*V)
        return h

    def get_value(self, x):
        return self.critic(self._sem_embed(x))
...
if __name__ == "__main__":
    ...
    agent = Agent(envs, sem_groups=args.sem_groups, sem_vertices=args.sem_vertices,
    ...
    if all_episode_returns:
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            w.writerow([args.env_id, args.seed, args.hypothesis_id, args.experiment_id, "ppo_sem",
