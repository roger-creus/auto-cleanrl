# IQN + OQE + Higher Replay Ratio + Periodic Soft Resets for Atari using envpool
# Combines the two strongest IQN innovations:
#   1. OQE (Optimistic Quantile Exploration) from h066: during action selection,
#      sample τ from U(β_t, 1.0) where β_t anneals 0.5→0.0. Optimistic exploration
#      early, exploitation late. Uses IQN's distributional info for principled exploration.
#   2. Higher Replay Ratio + Resets from h067: 4x gradient updates per env step
#      + periodic soft resets of value head every 200K steps (shrink 80%).
# These are orthogonal: OQE improves exploration quality, replay ratio improves
# sample efficiency, resets maintain plasticity. They should compose well.
import csv
import os
import random
import time
...
class Args:
    ...
    # Higher replay ratio + periodic resets
    updates_per_step: int = 4
    """number of gradient updates per env step batch (default 1 in base IQN, 4 here for RR≈2)"""
    reset_interval: int = 200000
    """soft-reset the value head every N global_steps to maintain plasticity"""
    reset_fraction: float = 0.8
    """fraction to shrink value head weights toward zero during reset (0.8 = keep 20%)"""

    # results tracking
    hypothesis_id: str = "h068"
...
if __name__ == "__main__":
    ...
    while global_step < args.total_timesteps:
        # OQE + epsilon-greedy action selection
        ...
        with torch.no_grad():
            ...
            # OQE: sample τ from U(oqe_beta, 1.0) for optimistic exploration
        ...
        # Training with higher replay ratio
        if global_step > args.learning_starts:
            if (global_step // args.num_envs) % args.train_frequency == 0:
                # Multiple gradient updates per env step (higher replay ratio)
                for _update_idx in range(args.updates_per_step):
                    ...
                    with torch.no_grad():
                        ...
                        td_target = s_rewards.unsqueeze(1) + args.gamma * (1 - s_dones.unsqueeze(1)) * target_q
            ...
            # Periodic soft reset of value head to maintain plasticity
            if args.reset_interval > 0 and global_step % args.reset_interval == 0 and global_step > 0:
                with torch.no_grad():
                    q_network.value_head.weight.mul_(1.0 - args.reset_fraction)
                    q_network.value_head.bias.mul_(1.0 - args.reset_fraction)
    ...
    if all_episode_returns:
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            w.writerow([args.env_id, args.seed, args.hypothesis_id, args.experiment_id, "iqn_oqe_replay",
