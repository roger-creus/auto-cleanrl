# PPO + CVaR + Dueling Architecture + DrQ Augmentation
# Combines h029 (CVaR advantage estimation) with h020 (dueling value/advantage decomposition)
# Value stream: outputs quantile predictions (distributional) for CVaR
# Advantage stream: outputs per-action advantages used as policy logits
# This separates "how good is this state" (distributional value) from "which action is best" (advantage)
import csv
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, num_quantiles=32):
        ...
        # Dueling: value stream outputs quantiles (distributional)
        self.value_stream = nn.Sequential(
            layer_init(nn.Linear(512, 256)),
            nn.ReLU(),
            layer_init(nn.Linear(256, num_quantiles), std=1),
        )
        # Dueling: advantage stream outputs per-action advantages (used as policy logits)
    ...
    def get_value(self, x, cvar_alpha=1.0):
        ...
        quantiles = self.value_stream(self.network(x / 255.0))
...
if __name__ == "__main__":
    ...
    if all_episode_returns:
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            w.writerow([args.env_id, args.seed, args.hypothesis_id, args.experiment_id, "ppo_cvar_duel",
