# PPO + Spectral Normalization + Soft Shrink-and-Perturb
# Combination of h013 (specnorm) and h007 (S&P) — complementary plasticity methods
# SpectralNorm: constrains Lipschitz constant of CNN, prevents representation collapse
# S&P: post-update weight shrinkage + noise injection maintains plasticity in weight space
import csv
import os
import random
import time
...
class Args:
    ...
    # Soft Shrink-and-Perturb hyperparameters (applied to non-specnorm layers)
    sp_shrink: float = 1e-4
    """shrink coefficient — soft L2 toward origin applied post-step"""
    sp_perturb: float = 1e-5
    """perturbation noise std — Gaussian noise added post-step"""
...
class Agent(nn.Module):
    def __init__(self, envs):
        super().__init__()
        # SpectralNorm on Conv2d layers
        self.network = nn.Sequential(
            spectral_norm(nn.Conv2d(4, 32, 8, stride=4)),
            nn.ReLU(),
            spectral_norm(nn.Conv2d(32, 64, 4, stride=2)),
            nn.ReLU(),
            spectral_norm(nn.Conv2d(64, 64, 3, stride=1)),
            nn.ReLU(),
            nn.Flatten(),
...
if __name__ == "__main__":
    ...
    # Identify non-spectral-norm weight parameters for S&P
    # SpectralNorm wraps parameters as "weight_orig", so skip those
    sp_params = []
    for name, param in agent.named_parameters():
        if param.requires_grad and param.dim() >= 2 and "weight_orig" not in name:
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Soft Shrink-and-Perturb on non-spectral-norm weight matrices
                with torch.no_grad():
    ...
    if all_episode_returns:
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            w.writerow([args.env_id, args.seed, args.hypothesis_id, args.experiment_id, "ppo_specnorm_sp",
